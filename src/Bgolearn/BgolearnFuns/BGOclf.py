"""Classification-boundary sampling utilities for Bgolearn.
...
class Boundary(object):
    ...
    def __init__(
        ...
        self.probs = model.fit(data_matrix, Measured_response).predict_proba(
            scale_virtual_samples
        )
    ...
    def Least_cfd(self):
        ...
        if self.opt_num == 1:
            ...
            print(
                "The next datum recommended by Least confidence : \n x = ",
                self.virtual_samples[LcValue_index],
            )
            ...
            for j in range(len(LcValue_index)):
                print(
                    "The {num}-th datum recommended by Least confidence : \n x = ".format(
                        num=j + 1
                    ),
                    self.virtual_samples[LcValue_index[j]],
                )
    ...
    def Margin_S(self):
        ...
        if self.opt_num == 1:
            ...
            print(
                "The next datum recomended by Margin sampling : \n x = ",
                self.virtual_samples[Mg_index],
            )
            ...
            for j in range(len(Mg_index)):
                print(
                    "The {num}-th datum recomended by Margin sampling : \n x = ".format(
                        num=j + 1
                    ),
                    self.virtual_samples[Mg_index[j]],
                )
    ...
    def Entropy(
        ...
        """Rank candidates by predictive entropy.

        Returns:
            tuple: Entropy scores for all virtual samples and the recommended
            highest-entropy candidates.
        """
        Entropy_value = []
        for i in range(len(self.probs)):
            Etp = 0
        ...
        if self.opt_num == 1:
            ...
            print(
                "The next datum recomended by Entropy-based approach : \n x = ",
                self.virtual_samples[Etp_index],
            )
            ...
            for j in range(len(Etp_index)):
                print(
                    "The {num}-th datum recomended by Entropy-based approach : \n x = ".format(
                        num=j + 1
                    ),
                    self.virtual_samples[Etp_index[j]],
                )
