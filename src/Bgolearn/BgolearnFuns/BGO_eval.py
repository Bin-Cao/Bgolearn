"""Evaluation helpers for Bayesian global optimization workflows.
...
class BGO_Efficient(object):
    ...
    def __init__(
        self, Ture_fun, Def_Domain, Kriging_model, opt_num, ret_noise, min_search
    ):
    ...
    def Trail(
        ...
        pd.DataFrame(Iter_Num).to_csv(
            "./Bgolearn/Trail_{name}.csv".format(name=UTFs), encoding="utf-8-sig"
        )
    ...
    def Pdf(
        ...
        pd.DataFrame(diff).to_csv(
            "./Bgolearn/∆{name1}-{name2}.csv".format(name1=Ref_UTFs, name2=UTFs),
            encoding="utf-8-sig",
        )
        ...
        plt.ylabel("p.d.f.")
        plt.title(
            r"$\mu={mu}$,$\sigma={sigma}$,$P(>0)={prob}$".format(
                mu=round(mu, 4), sigma=round(sigma, 4), prob=round(prob, 4)
            )
        )
        plt.subplots_adjust(left=0.15)
        plt.tick_params(labelsize=16)
        plt.savefig(
            "./Bgolearn/∆{name1}-{name2}.png".format(name1=Ref_UTFs, name2=UTFs),
            dpi=800,
        )
        plt.show()
    ...
    def Count(
        ...
        print("\n")
        print(
            "After %f trails :" % trails
            + "%f times success; " % Success_Cot
            + "%f times failed" % (trails - Success_Cot)
        )
