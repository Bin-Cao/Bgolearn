"""Bayesian global optimization utilities for minimization.
...
class Global_min(object):
    """Bayesian global optimization model for minimization problems.
    ...
    def EI_log(self, T=None):
        """
        Logarithm Expected Improvement (EI) acquisition function for minimization.
    ...
    def EI_plugin(
        ...
        for i in range(len(self.virtual_samples_mean)):
            Var_Z = (
                cur_optimal_value - self.virtual_samples_mean[i]
            ) / self.virtual_samples_std[i]
            EIp = (cur_optimal_value - self.virtual_samples_mean[i]) * norm.cdf(
                Var_Z
    ...
    def Augmented_EI(self, alpha=1, tao=0):
        ...
        cur_opt_index = np.random.choice(
            np.flatnonzero(proposal_fun_value == proposal_fun_value.min())
        )
        ...
        for i in range(len(self.virtual_samples_mean)):
            ...
            EI = (cur_optimal_value - self.virtual_samples_mean[i]) * norm.cdf(
                Var_Z
            ) + self.virtual_samples_std[i] * norm_des(Var_Z)
    ...
    def EQI(self, beta=0.5, tao_new=0):
        ...
        for i in range(len(self.virtual_samples_mean)):
            ...
            EQI = (cur_optimal_value - __mean[i]) * norm.cdf(Var_Z) + __std[
                i
            ] * norm_des(Var_Z)
            EQI_list.append(EQI)
    ...
    def Reinterpolation_EI(
        ...
        for i in range(len(__y)):
            ...
            REI = (cur_optimal_value - __y[i]) * norm.cdf(Var_Z) + __std[i] * norm_des(
                Var_Z
            )
            REI_list.append(REI)
    ...
    def PoI(self, tao=0, T=None):
        ...
        if tao < 0:
            ...
            PoI_list = norm.cdf(
                (cur_optimal_value - self.virtual_samples_mean)
                / self.virtual_samples_std
            )
    ...
    def Thompson_sampling(
        ...
        index = np.where(
            np.array(optimal_value_set) == np.array(optimal_value_set).min()
        )[0][0]
        return (
    ...
    def Knowledge_G(self, MC_num=1, Proc_num: int = None):
        ...
        if not Proc_num:
            ...
            for x_value, v_sample_mean, v_sample_std in zip(
                ...
                MC_batch_min = self.__Knowledge_G_per_sample(
                    K_model,
                    MC_num,
                    self.virtual_samples,
                    v_sample_mean,
